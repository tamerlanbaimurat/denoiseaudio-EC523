...
if __name__ == "__main__":
    ...
    CHECKPOINT_DIR = './checkpoints_attention_ablation'
    ...
    resume_path = os.path.join(CHECKPOINT_DIR, "latest_no_attention.pt")
    ...
    for epoch in range(start_epoch + 1, EPOCHS + 1):
        ...
        if test_total < best_test_total:
            ...
            save_checkpoint(model, optimizer, scheduler, epoch, history,
                            os.path.join(CHECKPOINT_DIR, "best_no_attention.pt"))
            print(f"  -> New best test total: {best_test_total:.4f}")

        # Periodic checkpoint
        if epoch % SAVE_EVERY == 0:
            save_checkpoint(model, optimizer, scheduler, epoch, history,
                            os.path.join(CHECKPOINT_DIR, f"epoch{epoch:03d}.pt"))

        # Always save latest (for resume)
        save_checkpoint(model, optimizer, scheduler, epoch, history,
                        os.path.join(CHECKPOINT_DIR, "latest_no_attention.pt"))
